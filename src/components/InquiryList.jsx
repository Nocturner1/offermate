...
export default function InquiryList({ inquiries, setInquiries, hotels, onOpenOffer, onNewOffer, onDuplicateInquiry, availability, setAvailability }) {
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={e => { e.stopPropagation(); onDuplicateInquiry(inq) }}
            ...
            {filtered.map(inq => (
              ...
              >
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                  ...
                  {inq.totalAmount > 0 && <span className="font-semibold text-gray-800">{fmtCHF(inq.totalAmount)}</span>}
                  <div className="ml-auto flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    <button onClick={() => onDuplicateInquiry(inq)} className="text-gray-400 hover:text-blue-500 p-1" title="Duplizieren">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                      </svg>
                    </button>
                    <button onClick={() => deleteInquiry(inq.id, inq.customerName)} className="text-gray-400 hover:text-red-400 p-1" title="Löschen">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                      </svg>
                    </button>
